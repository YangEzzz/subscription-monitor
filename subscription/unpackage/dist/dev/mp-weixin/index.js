...
const pages_subscription_subscriptionData = require("./pages/subscription/subscription-data.js");
...
const _sfc_main = {
  ...
  computed: {
    ...
    statusFilters() {
      return [{ value: "default", label: "有效订阅" }, { value: "all", label: "全部状态" }, { value: "upcoming", label: "即将到期" }, { value: "pending", label: "待处理" }, { value: "paused", label: "已暂停" }, { value: "cancelled", label: "已取消" }, { value: "archived", label: "已归档" }];
    },
    activeStatusLabel() {
  ...
  methods: {
    ...
    loadLocalData() {
      ...
      this.subscriptions = Array.isArray(saved) && saved.length ? saved : pages_subscription_subscriptionData.createSeedSubscriptions();
    ...
    closeDetail() {
      this.activeView = this.previousView || "all";
      this.scrollToTop();
    },
    ...
    closeForm() {
      this.activeView = this.editingId ? "detail" : this.previousView || "home";
      this.formError = "";
      this.scrollToTop();
    },
    ...
    saveSubscription(force = false) {
      ...
      common_vendor.index.showToast({ title: this.editingId ? "修改已保存" : "订阅已添加", icon: "success" });
      this.activeView = "detail";
      this.editingId = null;
    ...
    processSubscription(action) {
      ...
      if (!item)
        return;
      if (action === "renewed") {
        item.nextBillingDate = pages_subscription_subscriptionData.getNextBillingDate(item.nextBillingDate, item.cycle);
        item.status = item.cycle === "一次性" ? "archived" : "active";
        common_vendor.index.showToast({ title: "已推算下次扣费日", icon: "success" });
    ...
    showMoreActions() {
      ...
      common_vendor.index.showActionSheet({ itemList: [isPaused ? "恢复订阅" : "暂停订阅", "复制订阅", "归档订阅"], success: (res) => {
        if (res.tapIndex === 0) {
          item.status = isPaused ? "active" : "paused";
          this.persist();
          common_vendor.index.showToast({ title: isPaused ? "订阅已恢复" : "订阅已暂停", icon: "none" });
        } else if (res.tapIndex === 1) {
          const copy = { ...item, id: Date.now(), name: `${item.name} 副本`, status: "active", createdAt: Date.now() };
          this.subscriptions.push(copy);
          this.persist();
          common_vendor.index.showToast({ title: "已复制订阅", icon: "success" });
        } else {
          item.status = "archived";
          this.persist();
          common_vendor.index.showToast({ title: "已归档", icon: "success" });
        }
      } });
    },
    deleteSubscription() {
    ...
    openReminderSettings() {
      this.activeView = "reminder-settings";
      this.scrollToTop();
    },
  ...
  return common_vendor.e({
    ...
    C: common_vendor.f($options.actionableReminders, (reminder, k0, i0) => {
      return {
        ...
        f: "3fa108be-8-" + i0,
        g: reminder.key,
        h: common_vendor.o(($event) => $options.handleReminder(reminder), reminder.key)
      };
    }),
    D: common_vendor.p({
      type: "right",
    ...
    F: common_vendor.o(($event) => $options.openForm(), "de")
  }) : $data.activeView === "all" ? common_vendor.e({
    H: common_vendor.t($options.visibleSubscriptions.length),
    I: common_vendor.p({
      type: "bars",
      size: "22",
      color: "#242a26"
    }),
    J: common_vendor.o((...args) => $options.chooseSort && $options.chooseSort(...args), "6f"),
    K: common_vendor.p({
      type: "search",
      size: "19",
      color: "#8c938e"
    ...
    M: common_vendor.o(common_vendor.m(($event) => $data.searchKeyword = $event.detail.value, {
      trim: true
    }), "f0"),
    ...
    P: common_vendor.o(($event) => $data.searchKeyword = "", "0c")
  } : {}, {
    Q: common_vendor.f($options.categoryFilters, (filter, k0, i0) => {
      return {
    ...
    R: common_vendor.f($options.statusFilters, (status, k0, i0) => {
      return {
    ...
    S: common_vendor.t($options.sortLabel),
    T: common_vendor.t($options.activeStatusLabel),
    ...
    Y: common_vendor.o((...args) => $options.resetFilters && $options.resetFilters(...args), "9b")
  }, {
    Z: common_vendor.p({
      type: "plus",
      size: "27",
    ...
    aa: common_vendor.o(($event) => $options.openForm(), "d3")
    ...
    ad: common_vendor.o((...args) => $options.goToday && $options.goToday(...args), "fc"),
    ae: common_vendor.p({
      type: "left",
      size: "19",
      color: "#5d655f"
    }),
    af: common_vendor.o(($event) => $options.changeMonth(-1), "fc"),
    ag: common_vendor.t($options.calendarTitle),
    ah: common_vendor.p({
      type: "right",
      size: "19",
      color: "#5d655f"
    }),
    ai: common_vendor.o(($event) => $options.changeMonth(1), "b3"),
    aj: common_vendor.f($data.weekdays, (day, k0, i0) => {
      return {
        a: common_vendor.t(day),
        b: day
      };
    }),
    ak: common_vendor.f($options.calendarDays, (day, k0, i0) => {
      return common_vendor.e({
    ...
    al: common_vendor.t($options.selectedDateTitle),
    am: common_vendor.t($options.selectedWeekday),
    an: common_vendor.t($options.selectedDateSubscriptions.length),
    ao: common_vendor.t($options.formatMoney($options.selectedDateTotal)),
    ap: $options.selectedDateSubscriptions.length
  }, $options.selectedDateSubscriptions.length ? {
    aq: common_vendor.f($options.selectedDateSubscriptions, (item, k0, i0) => {
      return {
        a: "3fa108be-20-" + i0,
        b: common_vendor.p({
          item
    ...
    ar: common_vendor.o(($event) => $options.openForm($data.selectedDate), "f7")
  }) : $data.activeView === "stats" ? {
    at: common_vendor.f($data.statPeriods, (period, k0, i0) => {
      return {
    ...
    av: common_vendor.t($options.statsLabel),
    aw: common_vendor.t($options.formatMoney($options.statsTotal)),
    ax: common_vendor.t($options.statsSubscriptionCount),
    ay: common_vendor.t($data.statsPeriod === "year" ? "年度" : "月均"),
    az: $options.donutBackground,
    aA: common_vendor.f($options.categoryStats, (item, k0, i0) => {
      return {
        a: item.color,
    ...
    aB: common_vendor.f($options.trendData, (bar, k0, i0) => {
      return {
    ...
    aE: common_vendor.t($data.subscriptions.length),
    aF: common_vendor.p({
      type: "notification",
      size: "18",
      color: "#177e4b"
    }),
    aG: common_vendor.t($data.settings.notificationEnabled ? "已模拟开启" : "未开启"),
    aH: $data.settings.notificationEnabled,
    aI: common_vendor.o(($event) => $options.setNotification($event.detail.value), "3d"),
    aJ: common_vendor.p({
      type: "calendar",
      size: "18",
      color: "#bb6b18"
    }),
    aK: common_vendor.t($data.settings.defaultReminders.join("、")),
    aL: common_vendor.t($data.settings.reminderTime),
    aM: common_vendor.p({
      type: "right",
      size: "17",
      color: "#aab0ac"
    }),
    aN: common_vendor.o((...args) => $options.openReminderSettings && $options.openReminderSettings(...args), "89"),
    aO: common_vendor.p({
      type: "email",
      size: "18",
      color: "#3c7fc1"
    }),
    aP: $data.settings.weeklySummary,
    aQ: common_vendor.o(($event) => $options.updateSetting("weeklySummary", $event.detail.value), "f5"),
    aR: common_vendor.p({
      type: "wallet",
      size: "18",
      color: "#6458c9"
    }),
    aS: common_vendor.t($data.settings.defaultCurrency),
    aT: common_vendor.p({
      type: "right",
      size: "17",
      color: "#aab0ac"
    ...
    aU: $data.currencies,
    aV: common_vendor.o(($event) => $options.updateSetting("defaultCurrency", $data.currencies[$event.detail.value]), "34"),
    aW: common_vendor.p({
      type: "download",
      size: "18",
      color: "#177e4b"
    ...
    aY: common_vendor.o((...args) => $options.exportData && $options.exportData(...args), "16"),
    aZ: common_vendor.p({
      type: "locked",
      size: "18",
      color: "#3c7fc1"
    ...
    bb: common_vendor.o((...args) => $options.showPrivacy && $options.showPrivacy(...args), "ef"),
    bc: common_vendor.p({
      type: "refresh",
      size: "18",
      color: "#cc4b52"
    ...
    be: common_vendor.o((...args) => $options.resetDemoData && $options.resetDemoData(...args), "57")
    ...
    bh: common_vendor.o((...args) => $options.closeDetail && $options.closeDetail(...args), "5d"),
    bi: common_vendor.p({
      type: "more-filled",
      size: "24",
      color: "#202622"
    }),
    bj: common_vendor.o((...args) => $options.showMoreActions && $options.showMoreActions(...args), "ff"),
    bk: common_vendor.p({
      item: $options.selectedSubscription
    }),
    bl: common_vendor.t($options.selectedSubscription.name),
    bm: common_vendor.t($options.selectedSubscription.plan || $options.selectedSubscription.category),
    bn: common_vendor.t($options.statusText($options.selectedSubscription)),
    bo: common_vendor.n($options.getStatus($options.selectedSubscription)),
    bp: common_vendor.t($options.selectedSubscription.amount === null ? "金额待补充" : $options.formatMoney($options.selectedSubscription.amount)),
    bq: common_vendor.t($options.formatDate($options.selectedSubscription.nextBillingDate)),
    br: common_vendor.t($options.daysText($options.selectedSubscription)),
    bs: common_vendor.t($options.selectedSubscription.cycle),
    bt: common_vendor.t($options.selectedSubscription.payment),
    bv: common_vendor.t($options.selectedSubscription.category),
    bw: common_vendor.t($options.selectedSubscription.autoRenew ? "已开启" : "未开启"),
    bx: common_vendor.t($data.settings.notificationEnabled ? "通知可用" : "仅站内提醒"),
    by: common_vendor.f($options.selectedSubscription.reminders, (day, k0, i0) => {
      return {
        a: common_vendor.t(day === 0 ? "当天" : `提前 ${day} 天`),
        b: day
      };
    }),
    bz: common_vendor.t($options.nextReminderText($options.selectedSubscription)),
    bA: $options.selectedSubscription.note || $options.selectedSubscription.cancelGuide
  }, $options.selectedSubscription.note || $options.selectedSubscription.cancelGuide ? common_vendor.e({
    bB: $options.selectedSubscription.note
  }, $options.selectedSubscription.note ? {
    bC: common_vendor.t($options.selectedSubscription.note)
  } : {}, {
    bD: $options.selectedSubscription.cancelGuide
  }, $options.selectedSubscription.cancelGuide ? {
    bE: common_vendor.t($options.selectedSubscription.cancelGuide)
  } : {}) : {}, {
    bF: !["cancelled", "archived"].includes($options.selectedSubscription.status)
  }, !["cancelled", "archived"].includes($options.selectedSubscription.status) ? {
    bG: common_vendor.p({
      type: "checkbox-filled",
      size: "20",
      color: "#177e4b"
    }),
    bH: common_vendor.o(($event) => $options.processSubscription("renewed"), "36"),
    bI: common_vendor.p({
      type: "calendar",
      size: "20",
      color: "#aa681c"
    }),
    bJ: common_vendor.o(($event) => $options.processSubscription("later"), "10"),
    bK: common_vendor.p({
      type: "closeempty",
      size: "21",
      color: "#c5444c"
    }),
    bL: common_vendor.o(($event) => $options.processSubscription("cancelled"), "be")
  } : {}, {
    bM: common_vendor.p({
      type: "trash",
      size: "19",
    ...
    bN: common_vendor.o((...args) => $options.deleteSubscription && $options.deleteSubscription(...args), "11"),
    bO: common_vendor.p({
      type: "compose",
      size: "19",
      color: "#ffffff"
    }),
    bP: common_vendor.o(($event) => $options.openForm(null, $options.selectedSubscription), "cf")
    ...
    bS: common_vendor.o((...args) => $options.closeForm && $options.closeForm(...args), "59"),
    bT: common_vendor.t($data.editingId ? "编辑订阅" : "新增订阅"),
    bU: !$data.editingId
  }, !$data.editingId ? {
    bV: common_vendor.f($data.serviceTemplates, (item, k0, i0) => {
      return {
    ...
    bW: $data.form.name,
    bX: common_vendor.o(common_vendor.m(($event) => $data.form.name = $event.detail.value, {
      trim: true
    }), "b1"),
    bY: $data.form.plan,
    bZ: common_vendor.o(common_vendor.m(($event) => $data.form.plan = $event.detail.value, {
      trim: true
    }), "2f"),
    ca: common_vendor.t($data.form.category),
    cb: common_vendor.p({
      type: "right",
      size: "16",
      color: "#a2a7a3"
    ...
    cc: $data.categories,
    cd: common_vendor.o(($event) => $data.form.category = $data.categories[$event.detail.value], "2e"),
    ce: common_vendor.f($data.logoColors, (color, k0, i0) => {
      return {
        a: color,
    ...
    cf: $data.form.amount,
    cg: common_vendor.o(($event) => $data.form.amount = $event.detail.value, "3b"),
    ch: common_vendor.t($data.form.cycle),
    ci: common_vendor.p({
      type: "right",
      size: "16",
      color: "#a2a7a3"
    ...
    cj: $data.cycles,
    ck: common_vendor.o(($event) => $data.form.cycle = $data.cycles[$event.detail.value], "f7"),
    cl: common_vendor.t($data.form.payment),
    cm: common_vendor.p({
      type: "right",
      size: "16",
      color: "#a2a7a3"
    ...
    cn: $data.payments,
    co: common_vendor.o(($event) => $data.form.payment = $data.payments[$event.detail.value], "95"),
    cp: common_vendor.t($options.formatDate($data.form.nextBillingDate)),
    cq: common_vendor.p({
      type: "right",
      size: "16",
      color: "#a2a7a3"
    ...
    cr: $options.todayKey,
    cs: $data.form.nextBillingDate,
    ct: common_vendor.o(($event) => $data.form.nextBillingDate = $event.detail.value, "c1"),
    cv: $data.form.autoRenew,
    cw: common_vendor.o(($event) => $data.form.autoRenew = $event.detail.value, "44"),
    cx: common_vendor.f($data.reminderOptions, (option, k0, i0) => {
      return common_vendor.e({
    ...
    cy: common_vendor.t($options.formReminderPreview),
    cz: $data.form.note,
    cA: common_vendor.o(common_vendor.m(($event) => $data.form.note = $event.detail.value, {
      trim: true
    }), "93"),
    cB: $data.form.cancelGuide,
    cC: common_vendor.o(common_vendor.m(($event) => $data.form.cancelGuide = $event.detail.value, {
      trim: true
    }), "3a"),
    cD: $data.formError
  }, $data.formError ? {
    ...
    cF: common_vendor.t($data.formError)
  } : {}, {
    cG: common_vendor.p({
      type: "checkmarkempty",
      size: "20",
    ...
    cH: common_vendor.t($data.editingId ? "保存修改" : "保存订阅"),
    cI: common_vendor.o((...args) => $options.saveSubscription && $options.saveSubscription(...args), "0d")
    ...
    cL: common_vendor.o(($event) => $data.activeView = "profile", "b6"),
    cM: common_vendor.p({
    ...
    cO: common_vendor.t($data.settings.timezone),
    cP: common_vendor.t($data.settings.reminderTime),
    cQ: common_vendor.p({
      type: "right",
      size: "16",
      color: "#a2a7a3"
    }),
    cR: $data.settings.reminderTime,
    cS: common_vendor.o(($event) => $options.updateSetting("reminderTime", $event.detail.value), "aa"),
    cT: common_vendor.o(($event) => $data.activeView = "profile", "37")
  } : {}, {
    G: $data.activeView === "all",
    ab: $data.activeView === "calendar",
    as: $data.activeView === "stats",
    aD: $data.activeView === "profile",
    bf: $data.activeView === "detail" && $options.selectedSubscription,
    bQ: $data.activeView === "form",
    cJ: $data.activeView === "reminder-settings",
    cU: $data.scrollTop,
    cV: $options.showTabBar
  }, $options.showTabBar ? {
    cW: common_vendor.f($data.tabs, (tab, k0, i0) => {
      return {
        a: "3fa108be-54-" + i0,
        b: common_vendor.p({
