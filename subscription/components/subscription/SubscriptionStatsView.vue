<template>
	<view class="page stats-page">
		...
		<stat-summary-card :currencies="currencies" :stats-currency="statsCurrency" :stats-label="statsLabel" :stat-periods="statPeriods" :stats-period="statsPeriod" :stats-total="statsTotal" :stats-subscription-count="statsSubscriptionCount" :format-money="formatMoney" @currency-change="$emit('currency-change', $event)" @period-change="$emit('period-change', $event)" />
		<view class="chart-card category-chart-card"><view class="card-heading"><text class="section-title">分类构成</text><text>{{ statsPeriod === 'year' ? '年度' : statsPeriod === 'next' ? '未来 30 天' : '月均' }}</text></view><view class="category-chart-body"><view class="donut" :style="{ background: donutBackground }"><view class="donut-hole"><text class="donut-value">{{ statsPeriod === 'year' ? '年度' : '月均' }}</text><text class="donut-label">分类占比</text></view></view><view class="legend"><view v-for="item in categoryStats" :key="item.name" class="legend-row"><view class="legend-color" :style="{ backgroundColor: item.color }"></view><text class="legend-name">{{ item.name }}</text><text class="legend-value">{{ formatMoney(item.value, statsCurrency) }}</text><text class="legend-percent">{{ item.percent }}%</text></view></view></view></view>
