<template>
  <view class="page list-page">
    <view
      ...
      ><text class="page-title">全部订阅</text></view
    >
    <view class="search-box"
      ><uni-icons type="search" size="19" color="#8c938e" /><input
        v-model="searchDraft"
        aria-label="搜索服务或套餐"
        :focus="searchFocused"
        ...
        confirm-type="search"
        placeholder="搜索服务或套餐"
        placeholder-class="placeholder" /><button
        role="button"
        tabindex="0"
        v-if="searchDraft"
        class="clear-search"
        ...
        @tap="clearSearch"
      >
        <uni-icons type="clear" size="18" color="#9ca19d" /></button
    ></view>
    <scroll-view class="filter-scroll" scroll-x :show-scrollbar="false"
      ><view class="filter-row"
        ><button
          ...
          @tap="$emit('category', filter.name)"
        >
          <text class="filter-label">{{ filter.name }}</text
    ...
    <button
      ...
      class="sort-summary"
      aria-label="选择排序方式"
      @tap="$emit('choose-sort')"
    >
      <view
        ><text>{{ sortLabel }}</text
        ><text class="sort-divider">·</text
        ><text>{{ activeStatusLabel }}</text></view
      ><uni-icons type="right" size="15" color="#89938c" />
    </button>
    <view v-if="visibleSubscriptions.length" class="subscription-list"
      ><button
        role="button"
        tabindex="0"
        v-for="item in visibleSubscriptions.slice(0, displayLimit)"
        :key="item.id"
        class="list-item"
        @tap="$emit('open-detail', item)"
      >
        <brand-logo :item="item" /><view class="list-main"
          ><view class="name-line"
            ><text class="list-name">{{ item.name }}</text
            ><text class="status-badge" :class="getStatus(item)">{{
              statusText(item)
            }}</text></view
          ...
          ><text class="list-date"
            >{{ formatDate(item.nextBillingDate) }} · {{ daysText(item) }}</text
          ></view
        ><uni-icons type="right" size="17" color="#b0b5b1" /></button
    ></view>
    <button
      role="button"
      tabindex="0"
      v-if="visibleSubscriptions.length > displayLimit"
      class="text-button green load-more"
      @tap="displayLimit += 20"
    >
      加载更多（已显示 {{ displayLimit }} / {{ visibleSubscriptions.length }}）
    </button>
    <empty-state-view
      v-if="!visibleSubscriptions.length && liveSubscriptions.length"
      icon="search"
      title="没有匹配的订阅"
    ...
    />
    <empty-state-view
      v-else-if="!liveSubscriptions.length"
      icon="plus"
      title="还没有添加订阅"
    ...
    <button
      ...
      @tap="$emit('open-form')"
    >
      <uni-icons type="plus" size="27" color="#ffffff" />
    </button>
  </view>
</template>
