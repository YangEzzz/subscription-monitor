<template>
  <view class="page membership-page">
    ...
    <view class="membership-hero" :class="{ active: isMember }">
      ...
      <view class="membership-hero-top">
        ...
        <view class="membership-hero-copy">
          <text class="membership-hero-tag">{{
            isMember ? "✦ 会员已激活" : "✦ 升级会员"
          }}</text>
          <text class="membership-hero-title">{{
            isMember ? "无限订阅已解锁" : "解锁无限订阅"
          }}</text>
          <text class="membership-hero-desc">{{
            isMember
              ? "感谢使用，所有会员权益均已开放"
              : "不再受 5 条免费额度限制，尽情记录"
          }}</text>
      ...
      <view class="hero-benefits">
        <view class="hero-benefit-item">
          <view class="hero-benefit-icon"
            ><uni-icons type="list" size="22" color="#c87f1a"
          /></view>
        ...
        <view class="hero-benefit-item">
          <view class="hero-benefit-icon"
            ><uni-icons type="notification-filled" size="22" color="#c87f1a"
          /></view>
        ...
        <view class="hero-benefit-item">
          <view class="hero-benefit-icon"
            ><uni-icons type="bars" size="22" color="#c87f1a"
          /></view>
    ...
    >
      <view class="membership-cta-inner">
        ...
        <view class="membership-cta-copy">
          ...
          <text class="membership-cta-sub">模拟开通 · 体验全部权益</text>
        </view>
    ...
    <view class="membership-section">
      ...
      <view class="benefit-list">
        <view class="benefit-row">
          <view class="benefit-icon-wrap gold"
            ><uni-icons type="list" size="20" color="#b8720f"
          /></view>
        ...
        <view class="benefit-row">
          <view class="benefit-icon-wrap green"
            ><uni-icons type="notification-filled" size="20" color="#16834d"
          /></view>
          <view class="benefit-copy">
        ...
        <view class="benefit-row">
          <view class="benefit-icon-wrap green"
            ><uni-icons type="bars" size="20" color="#16834d"
          /></view>
          <view class="benefit-copy">
        ...
        <view class="benefit-row no-border">
          <view class="benefit-icon-wrap green"
            ><uni-icons type="locked" size="20" color="#16834d"
          /></view>
          <view class="benefit-copy">
            <text class="benefit-name">演示服务存储</text>
            <text class="benefit-desc">数据存于演示服务内存，重启后重置</text>
          </view>
          <view class="benefit-badge all">免费可用</view>
        </view>
      </view>
      ...
      <view class="membership-status">
        <view class="status-card" :class="{ 'status-card-active': isMember }">
          <view class="status-card-icon-wrap" :class="{ gold: isMember }">
            <uni-icons
              :type="isMember ? 'vip-filled' : 'person'"
              size="22"
              :color="isMember ? '#b8720f' : '#16834d'"
            />
          </view>
          <view class="status-card-body">
            <text class="membership-status-label">当前方案</text>
            <text class="membership-status-value">{{
              isMember ? "会员版" : "免费版"
            }}</text>
            <text class="membership-status-note">{{
              isMember ? "全部权益已开放" : "基础功能可用"
            }}</text>
          </view>
        </view>
        <view class="status-card">
          ...
          <view class="status-card-body">
            ...
            <text class="membership-status-value">{{
              isMember ? "无限" : freeQuotaValue
            }}</text>
            <text class="membership-status-note">{{
              isMember ? "不受数量限制" : "5 条免费上限"
            }}</text>
          </view>
        </view>
      </view>
    ...
    <button
      ...
      tabindex="0"
      v-if="isMember"
      class="membership-secondary-cta"
      @tap="$emit('restore')"
    >
      恢复为免费版
    </button>
    ...
    <view class="membership-trust">
      <view class="trust-item">
        <view class="trust-icon"
          ><uni-icons type="locked" size="18" color="#16834d"
        /></view>
        <text class="trust-label">仅模拟体验</text>
        <text class="trust-sub">数据存本机</text>
      </view>
      <view class="trust-divider"></view>
      <view class="trust-item">
        <view class="trust-icon"
          ><uni-icons type="wallet" size="18" color="#16834d"
        /></view>
      ...
      <view class="trust-item">
        <view class="trust-icon"
          ><uni-icons type="refresh" size="18" color="#16834d"
        /></view>
    ...
    <text class="membership-footnote"
      >当前会员状态由演示服务保存，不产生真实扣款；服务重启后重置。</text
    >
  </view>
</template>
